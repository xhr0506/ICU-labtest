...
(
SELECT icustay_id
,sum(duration_hours) as vent_duration_sum FROM ventdurations
group by icustay_id
order by icustay_id
)
, crrt_duration as
(
SELECT icustay_id
,sum(duration_hours) as crrt_duration_sum FROM crrtdurations
group by icustay_id
order by icustay_id
)
select ie.subject_id, ie.hadm_id, ie.icustay_id
  , case
	...
	,crrt_duration.crrt_duration_sum as crrt_duration
	,case
			when vent_duration.vent_duration_sum is not null then 1
			else 0
		end as vent
	,	vent_duration.vent_duration_sum as vent_duration
from icustays ie
left join cv
...
left join vent_duration
	on ie.icustay_id = vent_duration.icustay_id
left join crrt_duration
	on ie.icustay_id = crrt_duration.icustay_id
